...
public class AHA {
    ...
    public void print(){
         for(int i =list.size()-1;i>=0;i--){
             System.out.println( "-----");
             System.out.println(    ((Arbre) list.get(i)).code       );
             System.out.println(    ((Arbre) list.get(i)).poids       );
             if( ((Arbre) list.get(i)).feuille ){
                 System.out.println(((Feuille) list.get(i)).lettre);
             }
        }
         System.out.println( "-----");
    }
    
    //TODO : REFAIRE L'ininatialisation des varibles !!!! et l'ajout des nous interne dans la list
    public void modificationAHA(char c){
        Arbre q = null;
        //Si s n'est pas dans H
        if(!this.charIsPresent(c)){
            System.out.println("Nouveau charctere : "+c);
            //Soit Q le pere de la feuille spécila dans H
            q =special.pere;
            //Le fils gauche est la feuille spécial
            Arbre buff = special;
            //Le fils droit est une feuille s de poid 1 et on rajoute la feuille dans le hash map
    ...
    //Ennoncé
    //On ne peux que remonter dans la liste donc il faut seulement la remettre a jour a la fin
    private void traitement(Arbre q) {
        ...
        for(int i =q.posList-1;i>0;i--){
            ...
            if(i <=1){
                //On incremente son poids et on re vérifis qu'il ne viole personne
                q.pere.poids++;
            }
        }
    ...
    public String getCodeSpecialChar() {
        System.out.println("On lit le char spécial !");
        return special.code;
    }
    ...
    public void mAJ(Arbre n ,String s){
        ...
        if(s.contentEquals("first")){
            ...
            //Je me met a jour grace au code de mon pere qui est a jour et au code que mon pere me donne
 ...
 * SWAP :
 * 
 * On ajoute une nouvelle feuille ou on incremente : 
 * boucle de notre "arbre" courant  jusqu'a la racine
 *  on incrémente pere , si en remontant (de un ? ou jusqu'a la racine) on a violé personne on fait rien
 *                                  ON cherche le denrier mec qu'on a violé , on échange notre position avec lui
 * 
 * 
 */